				...
				<?php foreach ($matrix as $row => $columns) : ?>
					...
					<tr class="<?php echo $row_index % 2 == 0 ? '' : 'alt'; ?>" data-index="<?php echo $row_index; ?>">
						...
						<?php foreach ($columns as $key => $field_data): ?>
							...
							$vmsg = $variation->get_stock_quantity() ? sprintf(__('Only %s available', 'wc_bulk_variations'), $variation->get_stock_quantity()) : sprintf(__('Currently unavilable', 'wc_bulk_variations'));
							?>
							...
							<td>
								<?php if ($field_data) : ?>
									...
									<input type="hidden" name="order_info[<?php echo $cell_index; ?>][variation_data][<?php echo $column_attribute; ?>]" value="<?php echo $key; ?>" />
									<input type="hidden" name="order_info[<?php echo $cell_index; ?>][variation_data][<?php echo $row_attribute; ?>]" value="<?php echo $row; ?>" />
								<?php else : ?>
