...
if (!class_exists('WC_Bulk_Variations_Compatibility')) :
	/**
	 ...
	 * @version 1.0
	 */
	class WC_Bulk_Variations_Compatibility {
		/**
		...
		public static function wc_attribute_label($label) {
			...
			if (self::is_wc_version_gte_2_1()) {
				return ucwords(wc_attribute_label($label));
			} else {
				global $woocommerce;
		...
		public static function wc_add_error($message){
			self::wc_add_notice($message, 'error');
		}
